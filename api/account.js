...
const accountInfo = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res.json({
        error: null,
        result: accountToApiType(await getAccountById(_id))
      })
    } catch {
      ...
      try {
        return res.json({
          error: null,
          result: accountToApiType(await getByPrimaryAddress(req.params.id))
        })
      } catch {
