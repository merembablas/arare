<template>
    <div>
        ...
        <div v-if="item" class="mt-2 md:m-10 h-full">
            <div
                class="flex flex-wrap h-full items-start justify-between w-full"
            >
                <div
                    class="
                        justify-center
...
export default {
    async asyncData({ params, $axios }) {
        ...
        const item = await $axios
            ...
            .then(({ data }) => {
                console.log(
                    '🚀 ~ file: _itemId.vue ~ line 163 ~ .then ~ data',
                    data
                )
                return data
            })
    ...
    methods: {
        saveHistory() {
            this.$axios
                .post(`/api/items/${this.item.id}/views`)
                .then((resp) => {
                    console.log(
                        '🚀 ~ file: _itemId.vue ~ line 161 ~ this.$axios.post ~ resp',
                        resp
                    )
                })
        }
